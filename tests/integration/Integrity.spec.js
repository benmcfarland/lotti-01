...
(0, vitest_1.describe)('CSV Repository Integrity Integration Test', () => {
    ...
    (0, vitest_1.it)('detects file corruption via SHA-256 mismatch', async () => {
        ...
        const repoWithExpectedHash = new CsvLotteryRepository_1.CsvLotteryRepository(testFile, originalHash);
        await (0, vitest_1.expect)(repoWithExpectedHash.load()).rejects.toThrow('CSV integrity check failed');
    });
    (0, vitest_1.it)('integrity check fails with corrupted file and correct hash expectation', async () => {
        const content = createCsvContent(testRecords);
        const hash = computeHash(content);
        // Save pristine data
        (0, fs_1.writeFileSync)(testFile, content, 'utf-8');
        ...
        await (0, vitest_1.expect)(repoStrictCheck.verifyIntegrity()).rejects.toThrow('CSV integrity check failed');
    });
